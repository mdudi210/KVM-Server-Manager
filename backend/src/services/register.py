...
def register(data: NewUser, claims = Depends(verify_admin)):
    ...
    try:
        with OpenDb() as cursor:
            cursor.execute(
                "INSERT INTO users (id, username, password, role) VALUES (%s,%s,%s,%s)", (str(user_id),str(username),str(hashed_password),role),
            )
    except Exception as e:
        logger.error(f"{claims.get("sub")} got {e} when creating creating for {username}")
